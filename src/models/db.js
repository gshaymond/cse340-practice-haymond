...
// Read the CA certificate content when it is available locally.
const caCertPath = path.join(__dirname, '../../bin', 'byuicse-psql-cert.pem');
const caCert = fs.existsSync(caCertPath) ? fs.readFileSync(caCertPath) : null;
...
if (nodeEnv.includes('dev') && process.env.ENABLE_SQL_LOGGING === 'true') {
    /**
